...
private
  ...
  def find_wishlists
    ...
    if term.to_s.size < 2
      @wishlists = Wishlist.joins("INNER JOIN (select count(*), wishlist_id from wishlist_entries group by wishlist_id having count(*) > 0) c on c.wishlist_id = wishlists.id").order('random()').limit(20)
    else
      term = "%#{term}%"
