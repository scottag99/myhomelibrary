module CommonCampaignActions
  ...
  def readers
    ...
    select_text = "teacher, reader_name, reader_age, reader_gender, grade, language_id, languages.name as language, is_consent_given, campaign_id, wishlists.id as id, COALESCE(book_count, 0) as book_count, is_delivered, grl, external_id"
    grouping = [:teacher, :reader_name, :reader_age, :reader_gender, :grade, :language_id, 'languages.name', :is_consent_given, "campaign_id", "wishlists.id", "book_count", :is_delivered, :grl, :external_id]
    if get_role == 'admin'
